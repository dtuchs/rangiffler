...
#### 1. Подними БД в докере

```posh
Dmitriis-MacBook-Pro rangiffler %  docker volume create rangiffler
Dmitriis-MacBook-Pro rangiffler %  docker pull mysql:8.0.33
Dmitriis-MacBook-Pro rangiffler %  docker run --name rangiffler-db -p 3306:3306 -e MYSQL_ROOT_PASSWORD=secret -v rangiffler:/var/lib/mysql -d mysql:8.0.33
```
...
#### 3. Запустите бэкенд rangiffler-auth

```posh
Dmitriis-MacBook-Pro rangiffler % ./gradlew :rangiffler-auth:bootRun
```

#### 5. Запустите бэкенд rangiffler-api

```posh
Dmitriis-MacBook-Pro rangiffler % ./gradlew :rangiffler-api:bootRun
```

Auth стартанет на порту 9000: http://127.0.0.1:9000/
Api стартанет на порту 8080: http://127.0.0.1:8080/
...
фронтом и rangiffler-api старый добрый REST. А вот взаимодействие между микросервисами можно
